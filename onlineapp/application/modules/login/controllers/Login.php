...
class Login extends MX_Controller {
	...
	$query=$this->db->get_where('t_user',array('email'=>$uname,'password'=>$passwd));
	...
	public function vemail()
	{
		...
		$query=$this->db->get_where('t_user',array("email" =>$uemail));
		$no=$query->num_rows();  
		if($no>=1)
		{
			$query=$this->db->get_where('t_user',array("email" =>$uemail));
	...
	public function  resetpassword()
	{
		...
		$this->Common_model->updwhere('t_user',array('email'=>$email),$userdata);
