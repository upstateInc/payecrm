...
class Pending_transactions extends CI_Controller 
{
	public function __construct() {
		...
		$this->viewfolder = 'pending_transactions/';
		$this->controllerFile = 'pending-transactions/';
		$this->namefile = 'pending_transactions';
	}
	...
	public function change_trans_type(){
		...
		if($value == 'Refund'){
			...
			if($totalRefundLeft < 0){
				echo 'Transaction is already Refunded';
				exit;
			}
			$grossPrice=0-$amount;
			unset($row['id']);
			...
			require_once($_SERVER['DOCUMENT_ROOT']."/system/routine/send_refund_invoice.php");			
		}
		/*******************/
		else if($value == 'Chargeback'){
			$chargeBackId=0;
			$chargeBackId=$this->db->query('SELECT id from '.$this->table.' where originalGatewayTransactionId='.$row['gatewayTransactionId'].' and status="Chargeback"')->row()->id;
			if($chargeBackId!=0){
				echo 'Transaction is already Chargedback';
				exit;
			}
		...
		$this->db->query("update ".$this->table."set attention_required='' where id=".$id."");
