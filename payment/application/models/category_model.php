...
class Category_model extends CI_Model {
	...
	public function get_all_menu($selected,$where_clause,$table) {
		...
		$this->db->from($table);
		$query = $this->db->get();
		$menu_dropdown = '<select name="parent_id" id="parent_id" style="width:210px;font-weight:normal"><option value="">-Root-</option>';
		if ($query->num_rows() > 0)
		{
	...
	public function get_all_category_list($table,$where_clause,$order_by_fld, $order_by) {
		$level = 0;
		...
		if ($query->num_rows() > 0)
		{
			...
			{ 
				...
				<td class="columnDataGrey" align="left">'.date('d M,Y',strtotime($row->rec_crt_date)).'</td>
				...
				$menu_llist .= '</td></tr>';
				$menu_llist .= $this->get_all_subcategory_list($row->id,$level,$table,$order_by_fld,$order_by);
			}
		}
		else
		{
	...
	public function get_all_subcategory_list($id,$level,$table,$order_by_fld,$order_by) {
		$level ++;
		$this->db->order_by($order_by_fld,$order_by);
		...
		if ($query->num_rows() > 0)
		{
			foreach ($query->result() as $row)
			{
				($row->status=='Y') ? $status = "<a href=".site_url($this->controllerFile.'/change_is_active/'.$row->id.'/N').">".active_icon()."</a>" : $status = "<a href=".site_url($this->controllerFile.'/change_is_active/'.$row->id.'/Y').">".inactive_icon()."</a>" ;
				...
				$submenu_list .= '</td></tr>';
				$submenu_list .= $this->get_all_subcategory_list($row->id,$level,$table,$order_by_fld,$order_by);
			}
	...
	public function get_all_category($selected,$where_clause,$table) {
		...
		$this->db->from($table);
		$query = $this->db->get();
		$cat_dropdown = '<select name="category_title" id="category_title" style="width:210px;font-weight:normal" value=""><option value="">-Select Category-</option>';
		if ($query->num_rows() > 0)
		{
			foreach ($query->result() as $row)
			{ 
				...
				$cat_dropdown .= $this->get_all_subcategory($row->id,$level,$selected,$table,$where_clause);
			}
	...
	public function get_all_subcategory($id,$level,$selected,$table,$where_clause) {
		$level ++;
		$this->db->order_by("order_id","ASC");
		$where_clause_2 = "parent_id ='".$id."'".$where_clause;
		$this->db->where($where_clause_2);
		...
		if ($query->num_rows() > 0)
		{
			...
			{ 
				...
				$subcat_dropdown .= $this->get_all_subcategory($row->id,$level,$selected,$table,$where_clause);
			}
