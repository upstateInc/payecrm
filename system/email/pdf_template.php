...
while($row = $product_query->fetch_array()){
	...
	exit;*/
					...
					if($productNameShow=='product'){
						$pdf_html .= '<td align="left" style="border:none;"><strong>'.$row['productName'].'</strong></td>';
					}					
						$pdf_html .= '<td align="left" style="border:none;"><strong>'.$productSubscriptionPeriodDisplay.'</strong></td>';
						$pdf_html .= '<td align="right" style="border:none;"><strong>$'.number_format($row['quantity']*$row['price_each'],2).'</strong></td>';
					$pdf_html .='</tr>';
				}
$pdf_html .='<tr><td align="left" colspan="3">Training and Support Services</td></tr>';
