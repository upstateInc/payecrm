...
<?php if($this->session->userdata('ADMIN_NOMINEEID')==""){ ?>
			<div class="form-group">
...
<!-------------------------------------------------------------->
            ...
            <table class="table" style="width:70%;">
              <thead>
                <tr>
					<th style="text-align:left"><a style="text-decoration:none;font-size:12px;font-weight:<?=($order_by_fld=='gatewayID')?'bold':'normal'?>;" href="javascript: hdnSort('gatewayID','<?php echo $order_by; ?>');">Gateway</a></th>
					
					<th style="text-align:left"><a style="text-decoration:none;font-size:12px;font-weight:<?=($order_by_fld=='')?'bold':'normal'?>;" href="javascript: hdnSort('','<?php echo $order_by; ?>');">Monthly Volume</a></th>
					<?php if($this->session->userdata('ADMIN_NOMINEEID')==""){?>
					...
					<!--th><a style="text-decoration:none;font-size:12px;font-weight:<?=($order_by_fld=='')?'bold':'normal'?>;" href="javascript: hdnSort('','<?php echo $order_by; ?>');">Chargeback</a></th-->
			...
			//if($companyID=="" && $gatewayName==""){
				//$gatewayView = $this->db->query("Select distinct(gatewayID) from  t_midmaster where ".$where_clause2." visibility='Y' order by gatewayID ASC");
			...
			if($companyID!=""){
				...
				$gatewayView = $this->db->query("Select distinct(gatewayID) from  t_midmaster where ".$where_clause2." gatewayId='".$gatewayName."' order by gatewayID ASC");
			}
			if($gatewayName!=""){
				$gatewayView = $this->db->query("Select distinct(gatewayID) from  t_midmaster where ".$where_clause2." gatewayId='".$gatewayName."' order by gatewayID ASC");
			}*/
			...
			<td style="color:#145A32;" align="left"> <?php echo '$'.number_format($monthly_limit,2);?> </td>
			<?php if($this->session->userdata('ADMIN_NOMINEEID')==""){?>
			<?php
			...
			$totCap=$this->db->query('SELECT sum(grossPrice) as sum , count(*) as cnt from '.$this->table.' where '.$where_clause1.' and status="Capture"')->row();
			
			echo '<td style="color:#145A32;" align="right">'; 
			$sumTot+=$totCap->sum; 
			$sumTotCnt+=$totCap->cnt;
			$totalCapture+=$totCap->sum;
			if($totCap->cnt > 0 )echo '$'.number_format($totCap->sum,2);
			echo '</td>';
			...
			/*echo '<td>'; 			
			if($totChargeback->cnt > 0 )echo number_format($totChargeback->sum,2).'('.$totChargeback->cnt.')';
			echo '</td>';*/
			
			$totSale=$this->db->query('SELECT sum(grossPrice) as sum , count(*) as cnt from '.$this->table.' where '.$where_clause1.' and status="Sale"')->row();
					
			echo '<td style="color:#145A32;" align="right">'; 
			$sumTot+=$totSale->sum; 
			$sumTotCnt+=$totSale->cnt;
			$totalSale+=$totSale->sum;
			if($totSale->cnt > 0 )echo '$'.number_format($totSale->sum,2);
			echo '</td>';
			}
			...
			/*echo '<td>'; 
			if($totVoid->cnt > 0 )echo number_format($totVoid->sum,2).'('.$totVoid->cnt.')';
			echo '</td>';
