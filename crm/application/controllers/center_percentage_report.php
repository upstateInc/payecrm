...
class Center_percentage_report extends CI_Controller 
{
	public function __construct() {
		...
		$this->viewfolder = 'center_percentage_report/';
		$this->controllerFile = 'center-percentage-report/';
		$this->namefile = 'center_percentage_report';
...
/**********************search*************************************/
	...
	public function download(){
	   ...
	   $objPHPExcel->getProperties()->setTitle("Center Percentage Report")->setDescription("none");
		...
		$objPHPExcel->getActiveSheet()->setCellValue('A1', 'Center Percentage Report');
		...
		if ($query->num_rows() > 0){
			...
			$objPHPExcel->getActiveSheet()->setCellValue('B'.$row, 'Refund');
			...
			$objPHPExcel->getActiveSheet()->setCellValue('D'.$row, 'Chargeback');
			...
			$objPHPExcel->getActiveSheet()->setCellValue('E'.$row, 'Changeback %');
			...
			$objPHPExcel->getActiveSheet()->setCellValue('F'.$row, 'Void');
			...
			$objPHPExcel->getActiveSheet()->setCellValue('G'.$row, 'Void %');
			...
			$objPHPExcel->getActiveSheet()->setCellValue('H'.$row, 'Failed');
			...
			$objPHPExcel->getActiveSheet()->setCellValue('I'.$row, 'Failed %');
			...
			$objPHPExcel->getActiveSheet()->setCellValue('J'.$row, 'Settlement');
			...
			$objPHPExcel->getActiveSheet()->setCellValue('K'.$row, 'Total Sale');
			...
			if($totRef->cnt > 0 && $totSettle->sum > 0){
				$refundPercentage=number_format(abs($totRef->sum/$totSettle->sum * 100),2).'%';
			}else{
				$refundPercentage="";
			}
	

			$objPHPExcel->getActiveSheet()->setCellValue('B'.$row, '-$'.number_format(abs($totRef->sum),2));
			$objPHPExcel->getActiveSheet()->getStyle('B'.$row)->getFont()->setBold(true);
			$objPHPExcel->getActiveSheet()->getStyle('B'.$row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);			
			$objPHPExcel->getActiveSheet()->setCellValue('C'.$row, $refundPercentage);
			$objPHPExcel->getActiveSheet()->getStyle('C'.$row)->getFont()->setBold(true);
			$objPHPExcel->getActiveSheet()->getStyle('C'.$row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);			
			/**********************Chargeback**************************/
			...
			if($totChargeback->cnt > 0 && $totSettle->sum > 0){
				$chargebackPercentage=number_format(abs($totChargeback->sum/$totSettle->sum * 100),2).'%';
			}else{
				$chargebackPercentage="";
			}			
			
			$objPHPExcel->getActiveSheet()->setCellValue('D'.$row, '-$'.number_format(abs($totChargeback->sum),2));
			$objPHPExcel->getActiveSheet()->getStyle('D'.$row)->getFont()->setBold(true);
			$objPHPExcel->getActiveSheet()->getStyle('D'.$row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);			
			
			$objPHPExcel->getActiveSheet()->setCellValue('E'.$row, $chargebackPercentage);
			$objPHPExcel->getActiveSheet()->getStyle('E'.$row)->getFont()->setBold(true);
			$objPHPExcel->getActiveSheet()->getStyle('E'.$row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);
			...
			if($totVoids->cnt > 0 && $totSettle->sum > 0){
				$voidPercentage=number_format(abs($totVoids->sum/$totSettle->sum * 100),2).'%';
			}else{
				$voidPercentage="";
			}			
			
			
			$objPHPExcel->getActiveSheet()->setCellValue('F'.$row, '$'.number_format($totVoids->sum,2));
			$objPHPExcel->getActiveSheet()->getStyle('F'.$row)->getFont()->setBold(true);
			$objPHPExcel->getActiveSheet()->getStyle('F'.$row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);

			$objPHPExcel->getActiveSheet()->setCellValue('G'.$row, $voidPercentage);
			$objPHPExcel->getActiveSheet()->getStyle('G'.$row)->getFont()->setBold(true);
			$objPHPExcel->getActiveSheet()->getStyle('G'.$row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);
			...
			if($totFailed->cnt > 0 && $totSettle->sum > 0){
				$failedPercentage=number_format(abs($totFailed->sum/$totSettle->sum * 100),2).'%';
			}else{
				$failedPercentage="";
			}
			
			$objPHPExcel->getActiveSheet()->setCellValue('H'.$row, '$'.number_format($totFailed->sum,2));
			$objPHPExcel->getActiveSheet()->getStyle('H'.$row)->getFont()->setBold(true);
			$objPHPExcel->getActiveSheet()->getStyle('H'.$row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);

			$objPHPExcel->getActiveSheet()->setCellValue('I'.$row, $failedPercentage);
			$objPHPExcel->getActiveSheet()->getStyle('I'.$row)->getFont()->setBold(true);
			$objPHPExcel->getActiveSheet()->getStyle('I'.$row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);			
			...
			$objPHPExcel->getActiveSheet()->setCellValue('J'.$row, '$ '.number_format($totSettle->sum,2));
			$objPHPExcel->getActiveSheet()->getStyle('J'.$row)->getFont()->setBold(true);
			$objPHPExcel->getActiveSheet()->getStyle('J'.$row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);			
			...
			$objPHPExcel->getActiveSheet()->setCellValue('K'.$row, '$ '.number_format($sumTot,2));
			$objPHPExcel->getActiveSheet()->getStyle('K'.$row)->getFont()->setBold(true);
			$objPHPExcel->getActiveSheet()->getStyle('K'.$row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);//$col++;
			$row++;
			}
			
			$sheet->getStyle("A".$row.":K".$row)->applyFromArray($border_style);
			$objPHPExcel->getActiveSheet()->setCellValue('A'.$row, 'Total');
			$objPHPExcel->getActiveSheet()->getStyle('A'.$row)->getFont()->setSize(14);
			$objPHPExcel->getActiveSheet()->getStyle('A'.$row)->getFont()->setBold(true);

			/***************************************************************/
			if($totalRefund != 0 && $totalSettle > 0) $totalRefundPercentage=number_format(abs($totalRefund/$totalSettle * 100),2).'%';
			if($totalChargeback != 0 && $totalSettle > 0) $totalChargebackPercentage= number_format(abs($totalChargeback/$totalSettle * 100),2).'%';
			if($totalVoids != 0 && $totalSettle > 0) $totalVoidsPercentage= number_format(abs($totalVoids/$totalSettle * 100),2).'%';
			if($totalFailed != 0 && $totalSettle > 0) $totalFailedPercentage= number_format(abs($totalFailed/$totalSettle * 100),2).'%';			
			
			/////////////////////////Refund//////////////////////////////////
			$objPHPExcel->getActiveSheet()->setCellValue('B'.$row, '-$'.number_format(abs($totalRefund),2));
			$objPHPExcel->getActiveSheet()->getStyle('B'.$row)->getFont()->setBold(true);
			$objPHPExcel->getActiveSheet()->getStyle('B'.$row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);
			
			$objPHPExcel->getActiveSheet()->setCellValue('C'.$row, $totalRefundPercentage);
			$objPHPExcel->getActiveSheet()->getStyle('C'.$row)->getFont()->setBold(true);
			$objPHPExcel->getActiveSheet()->getStyle('C'.$row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);			
			/////////////////////////Chargeback//////////////////////////////////
			$objPHPExcel->getActiveSheet()->setCellValue('D'.$row, '-$'.number_format(abs($totalChargeback),2));
			$objPHPExcel->getActiveSheet()->getStyle('D'.$row)->getFont()->setBold(true);
			$objPHPExcel->getActiveSheet()->getStyle('D'.$row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);
			
			$objPHPExcel->getActiveSheet()->setCellValue('E'.$row, $totalChargebackPercentage);
			$objPHPExcel->getActiveSheet()->getStyle('E'.$row)->getFont()->setBold(true);
			$objPHPExcel->getActiveSheet()->getStyle('E'.$row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);
			
			/////////////////////////Voids//////////////////////////////////
			$objPHPExcel->getActiveSheet()->setCellValue('F'.$row, '$ '.number_format($totalVoids,2));
			$objPHPExcel->getActiveSheet()->getStyle('F'.$row)->getFont()->setBold(true);
			$objPHPExcel->getActiveSheet()->getStyle('F'.$row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);
			
			$objPHPExcel->getActiveSheet()->setCellValue('G'.$row, $totalVoidsPercentage);
			$objPHPExcel->getActiveSheet()->getStyle('G'.$row)->getFont()->setBold(true);
			$objPHPExcel->getActiveSheet()->getStyle('G'.$row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);
			
			/////////////////////////Failed//////////////////////////////////
			$objPHPExcel->getActiveSheet()->setCellValue('H'.$row, '$ '.number_format($totalFailed,2));
			$objPHPExcel->getActiveSheet()->getStyle('H'.$row)->getFont()->setBold(true);
			$objPHPExcel->getActiveSheet()->getStyle('H'.$row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);			
			$objPHPExcel->getActiveSheet()->setCellValue('I'.$row, $totalFailedPercentage);
			$objPHPExcel->getActiveSheet()->getStyle('I'.$row)->getFont()->setBold(true);
			$objPHPExcel->getActiveSheet()->getStyle('I'.$row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);

			/////////////////////////Settled//////////////////////////////////
			$objPHPExcel->getActiveSheet()->setCellValue('J'.$row, '$ '.number_format($totalSettle,2));
			$objPHPExcel->getActiveSheet()->getStyle('J'.$row)->getFont()->setBold(true);
			$objPHPExcel->getActiveSheet()->getStyle('J'.$row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);
			
			/////////////////////////Total//////////////////////////////////
			$objPHPExcel->getActiveSheet()->setCellValue('K'.$row, '$ '.number_format($totalGoodSale,2));
			$objPHPExcel->getActiveSheet()->getStyle('K'.$row)->getFont()->setBold(true);
			$objPHPExcel->getActiveSheet()->getStyle('K'.$row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);
		}
		...
		header("Content-Disposition: attachment; filename=center_Percentage_report".time().".xls");
		ob_end_clean();
