            ...
            <table class="table">
              <thead>
				...
				<?php } ?>	
					...
					<!--th><a style="text-decoration:none;font-size:12px;font-weight:<?=($order_by_fld=='gateway_descriptor')?'bold':'normal'?>;" href="javascript: hdnSort('gateway_descriptor','<?php echo $order_by; ?>');">Company Billing Name</a></th>
					...
					<th><a style="text-decoration:none;font-size:12px;font-weight:<?=($order_by_fld=='cardType')?'bold':'normal'?>;" href="javascript: hdnSort('cardType','<?php echo $order_by; ?>');"> Type</a></th>

					<!--th><a style="text-decoration:none;font-size:12px;font-weight:<?=($order_by_fld=='')?'bold':'normal'?>;" href="javascript: hdnSort('','');">Status<br/> Match</a></th-->
              ...
              <?php foreach ($query->result() as $row){   ?> 
				...
				<?php } ?>
					...
					<?php
						...
						$rowCenter=$this->DB2->query("select * from t_invoice where gatewayTransactionId='".$row->gatewayTransactionId."'")->row_array();
						//print_r($rowCenter);
						//echo $rowCenter['status']*/
						$transactionUpdate = $row_center['transactionUpdate'];
						$canCapture = $row_center['canCapture'];
						$canVoid 	= $row_center['canVoid'];
						$canRefund 	= $row_center['canRefund'];
						$canChargeback = $row_center['canChargeback'];
						if($row_center['status']==""){$clr="000";}
						//if($row->status==$rowCenter['status']){ $clr="#228B22";}
						//if($rowCenter['status']!="" && $row->status!=$rowCenter['status']){ $clr="0000FF";}
					?>
							
					<?php
						$rowGateway = $this->common_model->Retrive_Record_By_Where_Clause('t_midmaster',"gatewayID like '%".$row->gatewayID."%'");
						if($rowGateway['gatewayType']=='nmi' && $rowGateway['status']=='Y' && $row->status !='Chargeback'){?>
						<span id="bolt<?php echo $row->gatewayTransactionId; ?>" onclick='check_status("<?php echo $row->gatewayTransactionId; ?>","<?php echo $row->gatewayID;?>","<?php echo $row->companyID; ?>")' class="glyphicon glyphicon-flash bolt" style="color: <?php echo $clr;?>;"></span>
						<span class="statInfo" id="statInfo<?php echo $row->gatewayTransactionId; ?>"></span>
						<?php } 
						}?>						
					</td-->                
				 ...
				 <td>
					<?php
					//echo 'dasdasd'.$this->session->userdata('ADMIN_PERMISSION');
			  ...
			  <?php } }else{
                  ...
                  <!--div class="btn-group"-->
					...
					<!--------------------------------Emails---------------------------------->
						...
						<!--------------------------Refund Email------------------------------>
						<?php if($row->status=="Refund"){?>
                          <a href="javascript:;" class="btn btn-info btn-xs" data-toggle="modal" data-target="#send_refund_invoice" onclick="SetInvoiceId('<?php echo md5($row->id); ?>');"><span aria-hidden="true" class="fa fa-envelope-o"></span></a>
						  
						<?php }else{ ?> 
						<!------------------------General Emails------------------------------->
						  <a href="#" class="btn btn-info btn-xs" data-toggle="modal" data-target="#send_invoice" onclick="SetInvoiceId('<?php echo md5($row->id); ?>');"><span aria-hidden="true" class="fa fa-envelope-o"></span> </a>
						<?php } ?>
						</div>
						<!--------------------------------Emails---------------------------------->
					</td>
                    ...
                    <form id="resendInvoiceForm" method="POST" action="<?php echo $this->config->item('company_utils'); ?>resend_invoice.php" >
                        <div class="modal-body" id="">
							...
							<div class="form-group">
								...
								<input type="checkbox" id="emailType" name="emailType[]" value="4" onclick="showHideBlock();">
							</div>
							<!--------------------------------------------------------------------->
							<div id="billingCompanyShow" style="display:none;">
								<div class="form-group">
									<label >Phone Number</label>
									<select class="form-control" name="phoneNo" id="phoneNo" onchange="showHidePhn(this.value);">
										<option value="Blank">Blank</option>
										<option value="Customer">Customer</option>
										<option value="EnterNumber">Enter Number</option>
									</select>
								</div>
								<div id="phoneBlock" style="display:none;">
									<div class="form-group">
									<label >Phone No</label>
									<input type="text" class="form-control" id="phoneNoval" name="phoneNoval" placeholder="Phone No to be displayed in invoice" >
								</div>
								</div>
								<div class="form-group">
									<label >Product Name</label>
									<input type="text" class="form-control" id="ProductName" name="ProductName" placeholder="Email Product name to be displayed in invoice" >
								</div>
							</div>	
							<!--------------------------------------------------------------------->
							...
							<div class="form-group" id="customerMailSendBlock">
