...
/**********************search*************************************/		
		...
		if($this->session->userdata('name') != '')
		{
			...
			$where_clause .= "(fname LIKE '%".addslashes($name)."%' or lname LIKE '%".addslashes($name)."%') AND ";
			$data['name'] = $name;
		}
		...
		if($this->session->userdata('email') != '')
		{
			...
			$where_clause .= "email LIKE '%".addslashes($email)."%' AND ";
			$data['email'] = $email;
		}
/**********************search*************************************/
		//$where_clause .= "(type ='superadmin')";	
		$where_clause  = substr($where_clause, 0, -4);		
		$total_rows = $this->common_model->countAll($this->tableVW,$where_clause);
		$query = $this->common_model->get_all_records($this->tableVW, $where_clause,$order_by_fld,$order_by,$offset,$limit);
	...
	public function insert() {
			...
			$row['nomineeID'] = $this->input->post('nomineeID');
			$row['commission'] = $this->input->post('commission');*/
			$row['status'] = 'Y' ;
			...
			$adminType['adminTypeId'] = $this->input->post('adminTypeId');
			$adminType['adminId'] = $insert_id;
			$insert_adminTypeId = $this->common_model->addRecord('t_adminAdminType',$adminType);
			if($this->session->userdata('ADMINLEVEL')==2){
				$adminLevel['adminLevelId'] == 2;
			}else{
				$adminLevel['adminLevelId'] = $this->input->post('adminLevelId');
			}
			
			$adminLevel['adminId'] = $insert_id;
			$insert_adminLevelId = $this->common_model->addRecord('t_adminAdminLevel',$adminLevel);
			
			$adminId=$this->session->userdata('ADMIN_ID');
			$merchantId=$this->db->query("select merchantId from t_adminCompany where adminId=".$adminId)->row()->merchantId;
			if($merchantId!=""){
				$adminCompany['merchantId'] = $merchantId;
				$adminCompany['adminId'] = $insert_id;
				$insert_adminCompanyId = $this->common_model->addRecord('t_adminCompany',$adminCompany);				
			}
			else if($this->input->post('merchantId')!=""){
				$adminCompany['merchantId'] = $this->input->post('merchantId');
				$adminCompany['adminId'] = $insert_id;
				$insert_adminCompanyId = $this->common_model->addRecord('t_adminCompany',$adminCompany);
			}
	...
	function update() {
		...
		if($this->input->post('adminTypeId')!=""){
			$adminType['adminTypeId'] = $this->input->post('adminTypeId');
			$update_adminTypeId = $this->common_model->Update_Record_byField($adminType,'t_adminAdminType',$id,'adminId');
		}
		if($this->input->post('adminLevelId')!=""){
			$adminLevel['adminLevelId'] = $this->input->post('adminLevelId');
			$update_adminLevelId = $this->common_model->Update_Record_byField($adminLevel,'t_adminAdminLevel',$id,'adminId');
		}
		if($this->input->post('merchantId')!=""){
			$adminCompany['merchantId'] = $this->input->post('merchantId');
			$update_adminCompanyId = $this->common_model->Update_Record_byField($adminCompany,'t_adminCompany',$id,'adminId');			
		}		
...
/**********************search*************************************/
	...
	public function downloadpdf() {
		...
		$data["list_title"] = 'Admin Users';
		$html = $this->load->view($this->viewfolder.'/list_pdf',$data,true);
		pdf_create($html, 'superadmin');
		//$this->load->view('admin/admin_user/admin_user_list_pdf',$data);
	}
