...
<!------------------------search section------------------------>			
			...
			<?php if($this->session->userdata('ADMIN_GROUP_ID')==""){ ?>
			<div class="form-group">
				<!--a href="<?php echo base_url().$this->controllerFile.'download'; ?>" class="btn btn-primary"><i class="fa fa-align-justify"></i> Generate</a--> 
...
<!-------------------------------------------------------------->
            ...
            <table class="table" style="width:70%;">
              <thead>
                <tr>
					...
					<?php if($this->session->userdata('ADMIN_GROUP_ID')==""){ ?>
						<th style="text-align:right"><a style="text-decoration:none;font-size:12px;font-weight:<?=($order_by_fld=='')?'bold':'normal'?>;" href="javascript: hdnSort('','<?php echo $order_by; ?>');">Refund Fee</a></th>
					<?php } ?>
					...
					<?php if($this->session->userdata('ADMIN_GROUP_ID')==""){ ?>
						<th style="text-align:right"><a style="text-decoration:none;font-size:12px;font-weight:<?=($order_by_fld=='')?'bold':'normal'?>;" href="javascript: hdnSort('','<?php echo $order_by; ?>');">Chargeback Fee</a></th>
					
						<th style="text-align:right"><a style="text-decoration:none;font-size:12px;font-weight:<?=($order_by_fld=='')?'bold':'normal'?>;" href="javascript: hdnSort('','<?php echo $order_by; ?>');">Wire Fee</a>
					
						</th><th style="text-align:right"><a style="text-decoration:none;font-size:12px;font-weight:<?=($order_by_fld=='')?'bold':'normal'?>;" href="javascript: hdnSort('','<?php echo $order_by; ?>');">Processing Fee</a></th>					
					...
					<?php } ?>
					<?php if($this->session->userdata('ADMIN_GROUP_ID')!=""){ ?>
					<th style="text-align:right"><a style="text-decoration:none;font-size:12px;font-weight:<?=($order_by_fld=='')?'bold':'normal'?>;" href="javascript: hdnSort('','<?php echo $order_by; ?>');" >Commission</a></th>
					<?php } ?>
			...
			?>
				...
				<?php echo '$'.number_format($totRefundFee,2);?>
			<?php 
			...
			<?php if($totChargeback->cnt > 0){
				...
				//$totChrgbakFee = 0 - $totChrgbakFee;
				echo '$'.number_format($totChrgbakFee,2);
				//$sumTot+=$totChrgbakFee;
			...
			?>
			<!----------------------Commission Fee----------------->
			<td style="color:#145A32;" align="right" <?php if($this->session->userdata('ADMIN_GROUP_ID')==""){ ?> style="display:none;" <?php } ?>>
				<?php $groupcommission =$this->db->query("select commission from t_admin where id=".$this->session->userdata('ADMIN_ID')."")->row()->commission;
				if($invoice_typeVal=='Net'){
					
					$individualCommission=($totSettle->sum+$totRef->sum)*$groupcommission/100;
				}				
				if($invoice_typeVal=='Gross'){
					
					$individualCommission=($totSettle->sum)*$groupcommission/100;
				}
				if($individualCommission > 0)echo '$'.number_format($individualCommission,2);
				$groupTotCommission +=$individualCommission;
				?>
			</td>
			<?php
			}
			...
			<!---------------------------------------------->
				<tr bgcolor="#eeeeee">
					...
					<?php if($this->session->userdata('ADMIN_GROUP_ID')==""){ ?>				
						<td align="right" style="color:#E74C3C;"><?php if($totalRefund != 0)  echo '$'.number_format($sumTotalRefundFees,2); ?></td>
					<?php } ?>	
					<td align="right" style="color:#E74C3C;"><?php if($totalChargeback != 0)  echo '$'.number_format(abs($totalChargeback),2); ?></td>	
					
					<?php if($this->session->userdata('ADMIN_GROUP_ID')==""){ ?>
					<td align="right" style="color:#E74C3C;"><?php if($totalChargeback != 0)  echo '$'.number_format($sumTotalChargebackFee,2); ?></td>
					
					<td align="right" style="color:#E74C3C;"><?php if($sumTotalTransferFee != 0)  echo '$'.number_format($sumTotalTransferFee,2); ?></td>
					
					<td align="right" style="color:#E74C3C;"><?php if($sumTotalProcessingFee != 0)  echo '$'.number_format($sumTotalProcessingFee,2); ?></td>	
					

					<td align="right" style="color:#145A32;"><?php if($totalGoodSale != 0)  echo '$'.number_format($totalGoodSale,2); ?></td>
					<?php } ?>
					<?php if($this->session->userdata('ADMIN_GROUP_ID')!=""){ ?>
					<td align="right" style="color:#145A32;"><?php if($groupTotCommission != 0)  echo '$'.number_format($groupTotCommission,2); ?></td>
					<?php } ?>
				</tr>
              </tbody>
            </table>
