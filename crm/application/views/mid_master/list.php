<?php $this->load->view('header');?>
        ...
        <div class="row">
          ...
          <div class="x_panel">
							...
							<div class="col-md-3 col-sm-3 col-xs-12 margine_bottom_5">
									<select name="gatewayID" class="form-control">
										...
										<?php foreach ($gateway->result() as $row){?>
											<option <?php if($gatewayID==$row->gatewayName){ echo 'selected';}?> value="<?php echo $row->gatewayName; ?>"><?php echo $row->gatewayName; ?></option>
										<?php } ?>
									</select>
							</div>
							<div class="col-md-3 col-sm-3 col-xs-12 margine_bottom_5">
				...
				<option value="credit_card" <?php if($paymentType=='credit_card'){ echo 'selected'; }?>>Credit Card</option>
				<option value="echecking" <?php if($paymentType=='echecking'){ echo 'selected'; }?>>EChecking</option>
			</select>
							</div>
                      ...
                      <table class="table table-striped jambo_table bulk_action">
                        <thead>
                          <tr class="headings">
                            ...
                            <th class="column-title"><a style="text-decoration:none;font-size:12px;font-weight:<?=($order_by_fld=='gatewayID')?'bold':'normal'?>;" href="javascript: hdnSort('gatewayID','<?php echo $order_by; ?>');">Gateway</a> </th>
                            <th class="column-title"><a style="text-decoration:none;font-size:12px;font-weight:<?=($order_by_fld=='gatewayType')?'bold':'normal'?>;" href="javascript: hdnSort('gatewayType','<?php echo $order_by; ?>');">Gateway Type </a></th>
                            <th class="column-title"><a style="text-decoration:none;font-size:12px;font-weight:<?=($order_by_fld=='link')?'bold':'normal'?>;" href="javascript: hdnSort('link','<?php echo $order_by; ?>');">Link </a> </th>
						...
						?>
                          <tr class="<?php echo $rowType;?> pointer">
                            ...
                            <td class=" "><?php echo $row->gatewayID;?></td>
                            <td class=" "><?php echo $row->gatewayType;?></td>
                            
                            <td class=" "><a href="<?php echo $row->link;?>">Link</a></td>
