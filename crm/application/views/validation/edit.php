...
<div class="mainpanel">
    ...
    <!------------------------------------------Void/Refund---------------------------------------------------->
				...
				<?php
					$row_gateway = $this->common_model->Retrive_Record_By_Where_Clause('t_midmaster',"gatewayID like '%".$query['gatewayID']."%'");
					if($row_gateway['link']!=''){
				?>
						<a target="_blank" href="<?php echo $row_gateway['link'];?>"><h4>Gateway Link</h4></a>
					<?php }else{ ?>
						NA
					<?php } ?>
				   <!---------------------------------------------------------------------------------------------------------->
