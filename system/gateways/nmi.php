...
// Initial Setting Functions
  ...
  function doSale($amount, $ccnumber, $ccexp, $cvv="", $gatewayID, $agentName, $companyID) {
    ...
    // User Defined 	
    $query .= "merchant_defined_field_1=" . urlencode($gatewayID) . "&";
   // $query .= "merchant_defined_field_2=" . urlencode($agentID) . "&";
    $query .= "merchant_defined_field_2=" . urlencode($agentName) . "&";
    $query .= "merchant_defined_field_3=" . urlencode($companyID) . "&";	
    $query .= "type=sale";
    return $this->_doPost($query);
  }

function doAuth($amount, $ccnumber, $ccexp, $cvv="", $gatewayID, $agentName, $companyID) {
  ...
  function _doPost($query) {
    ...
    return $this->responses['response'];
		...
		$gw->setBilling($fname,$lname,"NA",$address,"", $city,$state,$zip,$country,$contact,"NA",$email, "NA");
					
		$gw->setShipping($fname,$lname,"NA",$address,"", $city,$state,$zip,$country,$contact,"NA",$email, "NA");
					
		$gw->setOrder($order_id,$product_name,1, 2, " "," " );
			

		if($tranactionMode=='Auth'){	
			$r = $gw->doAuth($product_price,$cardnumber,$month.$year,$cvv,$gatewayID,$agentName,$companyID);
		}
		if($tranactionMode=='Sale'){
			$r = $gw->doSale($product_price,$cardnumber,$month.$year,$cvv,$gatewayID,$agentName,$companyID);				
		}
