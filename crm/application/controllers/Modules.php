...
class Modules extends CI_Controller
{
	...
	public function editPerm()
	{
		...
		$getThisModuleAction = $this->db->query("SELECT actionId FROM t_moduleaction WHERE moduleId=".$id);
		$thisModuleAction =  $getThisModuleAction->result();
		
		#echo "<pre>"; print_r($thisModuleAction); exit;		
		
		$getAdminLevels = $this->db->query("SELECT * FROM t_adminLevel where status='Y'");
		$AdminLevels = $getAdminLevels->result();
		
		$getAdminType = $this->db->query("SELECT * FROM t_adminType where status='Y'");
		$AdminTypes = $getAdminType->result();
		...
		$data ['thisModuleAction'] 	= $thisModuleAction;
		
		$data ['AdminLevels'] 		= $AdminLevels;
		$data ['AdminTypes'] 		= $AdminTypes;
