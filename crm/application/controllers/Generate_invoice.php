...
class Generate_invoice extends CI_Controller 
{
	public function __construct() {
		...
		$this->namefile = 'generate_invoice';
	}
	public function generate(){
		...
		foreach ($companyIDName->result() as $row){
			...
			$arrinvoiceEmails = $rowCompany['invoiceEmails'];
				...
				//echo $end_date =  strtotime(date("Y-m-d", strtotime($yyyy_mm_dd)) . " +".$invoice_period."days");
				$calinvoice_period = $invoice_period - 1;
				
$newdate = strtotime($start_date);
$newdate1 = strtotime("+".$calinvoice_period."day", $newdate);
$end_date = date('m/d/Y', $newdate1);
				
				//exit;
				
				$parts = explode('/',$end_date);
				$yyyy_mm_dd3 = $parts[2] . '-' . $parts[0] . '-' . $parts[1];			
				$yyyy_mm_dd4 = $parts[2] . '/' . $parts[0] . '/' . $parts[1];			
				//$where_clause .= "`rec_up_date` <= ' ".$yyyy_mm_dd." 23:59:59' AND ";
				$where_clause = "companyID LIKE '%".$companyID."%' AND `rec_up_date` >= ' ".$yyyy_mm_dd." 00:00:00' AND `rec_up_date` <= ' ".$yyyy_mm_dd3." 23:59:59' ";
				$data['end_date'] = $end_date;
				
				//$where_clause  = substr($where_clause, 0, -4);
				$tempInvoiceGenerationId="";
				...
				if($totalInvoiceRecords > 0){
					$tempInvoiceGenerationId=$recordExist['tempInvoiceGenerationId'];
					$this->db->query("Delete from t_savedInvoice where tempInvoiceGenerationId ='".$tempInvoiceGenerationId."'");
				}else{
					$tempInvoiceGenerationId = time().rand(0,99);
				}
				...
				/**********************Bucket**************************/
				$settleCnt=$this->db->query('SELECT * from '.$this->table.' where '.$where_clause.' and status="Settlement"')->num_rows();
			$refCnt=$this->db->query('SELECT * from '.$this->table.' where '.$where_clause.' and status="Refund"')->num_rows();			
			$chrgbakCnt=$this->db->query('SELECT * from '.$this->table.' where '.$where_clause.' and status="Chargeback"')->num_rows();			
			
				$totGrossSale=$totSettle+$totRef+$totChrgBak;
			
			
			
			$refNo=$this->db->query('SELECT * from '.$this->table.' where '.$where_clause.' and status="Refund"')->num_rows();
				if($refNo!="" && $companyID!=""){
			...
			if($totChrgBak!=""){ 
			$chrgBackN0=$this->db->query('SELECT * from '.$this->table.' where '.$where_clause.' and status="Chargeback"')->num_rows();
				if($chrgBackN0!="" && $companyID!=""){
			...
			//echo $tempInvoiceGenerationId;
			//exit;
			$rowInsert['tempInvoiceGenerationId'] = $tempInvoiceGenerationId;
			
		$where_clause1 = "tempInvoiceGenerationId like '%".$tempInvoiceGenerationId."%'";
		$query1 = $this->common_model->get_all_records('t_invoiceDebitCredit', $where_clause1,'id','desc',$offset,$limit);
		...
		//$row1 = $this->common_model->Retrive_Record_By_Where_Clause1('t_savedInvoice',$where_clause1);
		//$savedInvoiceIdExists=0;
		/*echo $savedInvoiceIdExists=$this->db->query("select id from t_savedInvoice where tempInvoiceGenerationId='".$tempInvoiceGenerationId."'")->row_array()->id;
		if($savedInvoiceIdExists!=0){
			$this->db->query("Delete from t_savedInvoice where tempInvoiceGenerationId ='".$tempInvoiceGenerationId."'");
		}*/
		$rowInsert['total_payout'] = number_format($NETPAYOUT - $moreTotalDeduction,2);
		$rowInsert['INVOICECOMPANYID'] = $companyID;
		$rowInsert['STARTDATE'] = $start_date;
		$rowInsert['NOOFDAYS'] = $invoice_period;
		$rowInsert['ENDDATE'] = $end_date;

		$rowInsert['COMMISSIONFEE'] = $COMMISSIONFEE;
		$rowInsert['TOTALSALE'] = number_format($totSettle,2);
		$rowInsert['TOTALGOODSALE'] = number_format($TOTALGOODSALE,2);
		$rowInsert['PROCESSINGFEE'] = number_format($totProcessingFee,2);
		$rowInsert['REFUNDEACH'] = $REFUNDFEEVAL;
		$rowInsert['NOREFUND'] = $refNo;
		$rowInsert['TOTALREFUND'] = number_format($totRefundFee,2);
		$rowInsert['CHARGEBACHEACH'] = $CHARGEBACHEACH;
		$rowInsert['NOCHARGEBACK'] = $chrgBackN0;
		$rowInsert['TOTALCHARGEBACK'] = $totChrgbakFee;
		$rowInsert['ACHFEE'] = number_format($totACHFee,2);
		$rowInsert['WIREFEE'] = number_format($totWireFee,2);

		$rowInsert['NETCHARGEBACK'] = number_format($totChrgBak,2);
		$rowInsert['NETREFUND'] = number_format($totRef,2);
		$rowInsert['TOTALGROSSSALE'] = number_format($totGrossSale,2);
		$rowInsert['NETDEDUCTION'] = number_format($netDeduction,2);
		$rowInsert['INVOICETYPE'] = $invoice_typeVal;
		$rowInsert['TOTALRESERVE'] = $totalReserve;
		$rowInsert['RESERVEPERCENTAGE'] = $reservePercentageVal;		
		//if($rowInsert['total_payout'] > 0){
		//echo $companyID.' - '.$NETPAYOUT.' - '.$totGrossSale.' - '.$totRefundFee.' - '.$totChrgbakFee.' - '.$totWireFee.' - '.$totProcessingFee.' - '.$totACHFee."<br/>";
			//if($NETPAYOUT > 0){
			if($totGrossSale > 0 || $refNo!="" || $totChrgBak!=""){	
				$insert_id = $this->common_model->addRecord('t_savedInvoice',$rowInsert);
		...
		//exit;
		$message = setMessage('Record created successfully.',"success");
...
/****************************************/		
	...
	public function volumeReport(){
		...
		$this->db->from($this->table);
		$query = $this->db->get();*/
		$query = $this->db->query("select DATE_FORMAT(rec_up_date,'%m-%d-%Y'),DATE_FORMAT(rec_crt_date,'%m-%d-%Y'),customer_name,agent_name,concat('$', format(grossPrice, 2)) from ".$this->table." where ".$where_clause." order by rec_up_date,rec_crt_date,customer_name ");
		
		$query1 = $this->db->query("select DATE_FORMAT(rec_up_date,'%m-%d-%Y'),DATE_FORMAT(rec_crt_date,'%m-%d-%Y'),customer_name,agent_name,concat('$', format(grossPrice, 2)) from ".$this->table." where ".$where_clause1." order by rec_up_date,rec_crt_date,customer_name ");
		
		$query2 = $this->db->query("select DATE_FORMAT(rec_up_date,'%m-%d-%Y'),DATE_FORMAT(rec_crt_date,'%m-%d-%Y'),customer_name,agent_name,concat('$', format(grossPrice, 2)) from ".$this->table." where ".$where_clause2." order by rec_up_date,rec_crt_date,customer_name ");
	...
	public function sendMailonDate(){
		$start_date	= $this->input->post('start_date');
		$recordExist=$this->db->query("select * from t_savedInvoice where STARTDATE like '%".$start_date."%'");
		foreach($recordExist->result() as $val){
			...
			$arrinvoiceEmails = $rowCompany['invoiceEmails'];			
			//print_r($val);
			$tempInvoiceGenerationId=$val->tempInvoiceGenerationId;
			$newwhere_clause = "tempInvoiceGenerationId like '%".$tempInvoiceGenerationId."%'";
	
	

			$query = $this->common_model->get_all_records('t_invoiceDebitCredit', $newwhere_clause,'id','desc',$offset,$limit);
			$data['query'] = $query ;

			$rowdata = $this->common_model->Retrive_Record_By_Where_Clause1('t_savedInvoice',$newwhere_clause);
			$data['row'] = $rowdata ;
			require_once($_SERVER['DOCUMENT_ROOT'].'/system/'."email/email_function.php");
			require_once($_SERVER['DOCUMENT_ROOT'].'/system/'."dompdf/dompdf_config.inc.php");
			...
			$emailContent = "Hello,<br>";
			$emailContent .= "Kindly find the Invoice Attached with this email.";
			$emailContent .= "The report is updated till today.";
			
			mail_file_attach2( $arrinvoiceEmails, 'Invoice', $emailContent, COMPANYEMAIL, $tempInvoiceGenerationId.'.pdf','','' );
			unlink($tempInvoiceGenerationId.'.pdf');			
		}
	}
}?>
