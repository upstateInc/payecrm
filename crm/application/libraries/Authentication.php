...
class Authentication {
	...
	function is_systemAdmin($session_param){
		if($session_param !='system'){
			ci_redirect(base_url().'dashboard');
			return false;			
		}		
	}
	function is_superAdmin($session_param){
		if($session_param !='superadmin' && $session_param !='teamlead'){
			ci_redirect(base_url().'dashboard');
			return false;			
		}
	}	
	function is_GroupSystemAdmin($session_param){
		if($session_param !='system' && $session_param !='group'){
			ci_redirect(base_url().'dashboard');
			return false;			
		}
	}	
	function is_NomineeSystemAdmin($session_param){
		if($session_param !='system' && $session_param !='nominee'){
			ci_redirect(base_url().'dashboard');
			return false;			
		}
	}	
